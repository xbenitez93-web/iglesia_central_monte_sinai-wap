...
export const RafflesModuleView: React.FC<RafflesModuleViewProps> = ({
  ...
  }, [raffles, filterTab, searchQuery, currentUser]);
  ...
  const handleOpenEditRaffle = (r: RaffleItem) => {
    ...
    setNewRaffleForm({
      title: r.title,
      category: r.category || 'Pro-Templo',
                ...
                <div>
                  ...
                  <div className="relative h-44 w-full bg-slate-800 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                      <span className="px-2.5 py-1 rounded-full text-[11px] font-black uppercase tracking-wider bg-black/60 text-amber-300 backdrop-blur-md border border-amber-500/30">
                        {raffle.category || 'Sorteo Eclesial'}
                      </span>
      ...
      {drumRaffle && (
        <div
          className={`fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center ${
            isDrumFullscreen ? 'p-0 w-screen h-screen' : 'p-3 sm:p-4 overflow-y-auto'
          } animate-fadeIn`}
        >
          <div
            className={`${
              isDrumFullscreen
                ? 'w-full h-full rounded-none border-none max-w-none flex flex-col justify-between p-4 sm:p-8 overflow-y-auto'
                : 'rounded-3xl border border-amber-500/40 p-5 sm:p-7 max-w-3xl w-full shadow-2xl space-y-5 max-h-[94vh] overflow-y-auto'
            } bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white text-center relative overflow-hidden`}
          >
            {/* Ambient Lighting / Glow */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-72 bg-amber-500/15 blur-3xl pointer-events-none" />
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center space-x-2.5 text-left">
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="font-black text-lg sm:text-xl text-white">Tómbola Digital en Vivo</h3>
                    {isDrumFullscreen && (
                      <span className="px-2 py-0.5 rounded-md bg-amber-500/20 border border-amber-500/40 text-amber-300 font-extrabold text-[10px] tracking-wider uppercase">
                        Modo Proyector
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-amber-300/80 truncate max-w-md">{drumRaffle.title}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className={`p-2 sm:px-3 rounded-xl border font-bold text-xs transition-all flex items-center space-x-1.5 cursor-pointer ${
                    isDrumFullscreen
                      ...
                      : 'bg-slate-800/90 border-slate-700 hover:border-amber-500/60 text-amber-300'
                  }`}
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {isDrumFullscreen ? 'Reducir' : 'Pantalla Completa / Proyector'}
                  </span>
                </button>
            ...
            {/* Prize Selector & Preview */}
            <div className="space-y-3 text-left">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <label className="block text-xs font-black uppercase tracking-wider text-amber-400">
                  Selecciona el Premio en Disputa:
                </label>
                <span className="text-[11px] text-slate-400">
                  {drumRaffle.prizes.filter((p) => Boolean(p.winnerTicketNumber)).length} de {drumRaffle.prizes.length} premios entregados
                </span>
              </div>
              ...
              <select
                ...
                className="w-full px-4 py-2.5 rounded-2xl bg-slate-800/90 border border-amber-500/30 text-white font-bold text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
              >
                {drumRaffle.prizes.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.place}º Lugar: {p.name} {p.winnerTicketNumber ? `(🏆 Ganado por boleto #${p.winnerTicketNumber})` : '⏳ Disponible'}
                  </option>
                ))}
              </select>

              {/* Selected Prize Card Preview */}
              {(() => {
                const currentPrize = drumRaffle.prizes.find((p) => p.id === selectedPrizeId);
                if (!currentPrize) return null;
                return (
                  <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-amber-500/20 via-orange-500/15 to-amber-500/20 border border-amber-500/40 flex flex-col sm:flex-row items-center justify-between gap-3 text-left">
                          ...
                          onClick={() =>
                            setPreviewZoomImage({
                              ...
                              subtitle:
                                ...
                                (currentPrize.valueEstimated
                                  ? `Valor estimado: $${currentPrize.valueEstimated}`
                                  : undefined),
                            })
                          }
                          className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden border-2 border-amber-400 shrink-0 shadow-lg cursor-pointer group hover:scale-105 transition-transform"
                          title="Clic para ver foto en pantalla completa"
                        >
                          <img
                          ...
                          <span className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded-md bg-black/80 text-[10px] font-black text-amber-300">
                            {currentPrize.place}º
                          </span>
                        </div>
                      ) : (
                        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-amber-500 text-white font-black text-2xl flex items-center justify-center shrink-0 shadow-md">
                          {currentPrize.place === 1 ? '🥇' : currentPrize.place === 2 ? '🥈' : '🥉'}
                        </div>
                      )}
                      <div className="min-w-0">
                        <p className="text-xs font-black uppercase tracking-wider text-amber-300 flex items-center space-x-1.5">
                        ...
                        <h4 className="text-base sm:text-xl font-black text-white">{currentPrize.name}</h4>
                        {currentPrize.description && (
                          <p className="text-xs text-slate-300 line-clamp-2">{currentPrize.description}</p>
                        )}
                        {currentPrize.valueEstimated ? (
                    ...
                    {currentPrize.winnerTicketNumber && (
                      <span className="px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-black shrink-0">
                        ¡Ya Asignado!
                      </span>
                    )}
                  </div>
            ...
            {/* PREVENT REPEAT WINNERS & PARTICIPANT FILTER CONTROLS */}
            <div className="p-3 sm:p-4 rounded-2xl bg-slate-800/80 border border-slate-700 space-y-2.5 text-left">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                ...
                <label className="relative inline-flex items-center cursor-pointer select-none">
                  ...
                  <span className="ml-2.5 text-xs font-black text-amber-300 flex items-center space-x-1.5">
                ...
                <label className="relative inline-flex items-center cursor-pointer select-none">
                  ...
                  <span className="ml-2.5 text-xs font-bold text-slate-300 flex items-center space-x-1.5">
                ...
                <label className="relative inline-flex items-center cursor-pointer select-none">
                  ...
                  <span className="ml-2.5 text-xs font-bold text-rose-300 flex items-center space-x-1.5">
                    <Ban className="w-3.5 h-3.5 text-rose-400" />
                    <span>Excluir ausentes (no repetir)</span>
                  </span>
                </label>
              </div>

              {/* Status and count badges strip */}
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-700/60 text-xs">
                <div className="flex items-center space-x-2 text-slate-300">
                  <input
                    ...
                    disabled={isDrumSpinning}
                    className="w-3.5 h-3.5 rounded text-amber-500 focus:ring-amber-500 bg-slate-800 border-slate-700 cursor-pointer"
                  />
                  <label htmlFor="chk-only-sold" className="cursor-pointer font-medium select-none text-[11px] sm:text-xs">
                    Solo boletos pagados ({(drumRaffle.tickets || []).filter((t) => t.status === 'sold').length})
                  </label>
                </div>

                <div className="flex flex-wrap items-center gap-2 text-[11px]">
                  <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-black">
                    🎯 {drumEligibleCount} boletos en tómbola
                  </span>
                  {drumExcludedCount > 0 && drumPreventRepeatWinners && (
                    <span className="px-2.5 py-1 rounded-full bg-amber-500/20 border border-amber-500/30 text-amber-300 font-black flex items-center space-x-1">
                      <Trophy className="w-3 h-3" />
                      <span>{drumExcludedCount} ya premiados</span>
                    </span>
                  )}
                  {drumAbsentExcludedCount > 0 && (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-full bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 font-black flex items-center space-x-1 cursor-pointer transition-colors"
                      title="Ver y gestionar boletos excluidos"
                    >
                      <UserX className="w-3 h-3 text-rose-400" />
                      <span>{drumAbsentExcludedCount} descartados / ausentes</span>
                      <span className="underline ml-1 font-bold text-[10px]">
                        {showExcludedDrawer ? 'Ocultar' : 'Ver lista'}
                      </span>
                    </button>
                  )}
                </div>
              ...
              {/* Excluded Tickets In-Modal Drawer */}
              {showExcludedDrawer && (
                <div className="p-3.5 rounded-2xl bg-rose-950/40 border border-rose-800/60 space-y-2 text-left animate-fadeIn mt-2">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-black text-rose-200 uppercase tracking-wider flex items-center space-x-1.5">
                      <UserX className="w-3.5 h-3.5 text-rose-400" />
                      <span>Boletos Excluidos / Descalificados en este Sorteo</span>
                    </h4>
                    <button
                      type="button"
                  ...
                  <p className="text-[11px] text-rose-300/80">
                    Estos boletos salieron en la tómbola pero la persona no se presentó o fueron descalificados. Están bloqueados para no repetirse. Puedes restituirlos si se presentaron después.
                  </p>
                  <div className="max-h-48 overflow-y-auto divide-y divide-rose-900/40 space-y-1.5 pt-1">
                    {drumRaffle.tickets.filter((t) => t.isExcluded).length === 0 ? (
                      <p className="text-xs text-slate-400 py-2 italic text-center">No hay boletos excluidos actualmente.</p>
                    ) : (
                      drumRaffle.tickets
                        .filter((t) => t.isExcluded)
                        .map((t) => (
                          <div key={t.number} className="flex items-center justify-between pt-1.5 text-xs">
                            <div>
                              <span className="font-black text-white mr-2">{t.ticketCode}</span>
                              <span className="text-slate-300 font-medium">{t.buyerName || 'Participante'}</span>
                              <p className="text-[10px] text-rose-300 italic">
                                Motivo: {t.exclusionReason || 'No se presentó al llamado'}
                              </p>
                            </div>
                            <button
                              type="button"
                              onClick={() => handleRestoreExcludedTicket(t.number)}
                              className="px-2.5 py-1 rounded-xl bg-emerald-600/80 hover:bg-emerald-600 text-white font-bold text-[11px] flex items-center space-x-1 cursor-pointer transition-colors"
                              title="Volver a permitir que participe"
                            >
            ...
            {/* ANIMATED DRUM DISPLAY BOX */}
            <div
              className={`rounded-3xl bg-black/60 border-2 border-amber-500/50 shadow-inner relative overflow-hidden flex flex-col items-center justify-center space-y-4 transition-all ${
                isDrumFullscreen ? 'py-12 sm:py-16 px-8 my-auto' : 'py-7 px-6'
              }`}
            >
              <div className="text-xs sm:text-sm font-extrabold uppercase tracking-widest text-amber-400 flex items-center space-x-2">
                <Ticket className="w-4 h-4" />
                <span>Boleto Seleccionado</span>
              </div>

              {/* GIANT DIGITS BOX (PROJECTOR OPTIMIZED) */}
              <div
                className={`font-black font-mono tracking-widest px-8 sm:px-12 py-4 sm:py-6 rounded-3xl transition-all duration-300 select-none ${
                  isDrumFullscreen ? 'text-7xl sm:text-9xl md:text-[130px]' : 'text-6xl sm:text-7xl'
                } ${
                  isDrumSpinning
                    ? 'scale-105 text-amber-400 bg-amber-500/10 border border-amber-500/40 animate-pulse drop-shadow-[0_0_25px_rgba(245,158,11,0.5)]'
                    : drumCandidateWinner
                    ? 'scale-110 text-emerald-400 bg-emerald-500/20 border-2 border-emerald-500 shadow-2xl shadow-emerald-500/30 drop-shadow-[0_0_35px_rgba(16,185,129,0.5)]'
              ...
              {/* CANDIDATE WINNER DETAIL (PROMINENT CELEBRATION & DECISION CARD) */}
              {drumCandidateWinner && (
                <div className="animate-scaleUp space-y-3 p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-amber-500/20 via-emerald-500/20 to-amber-500/20 border-2 border-amber-400 w-full max-w-xl shadow-2xl">
                  <div className="flex items-center justify-center space-x-2 text-amber-300 font-black text-sm sm:text-base">
                    <PartyPopper className="w-5 h-5 text-amber-400" />
                    <span>¡NÚMERO EXTRAÍDO DE LA TÓMBOLA!</span>
                    <PartyPopper className="w-5 h-5 text-amber-400" />
                  </div>

                  <div className="bg-black/40 rounded-2xl p-3 border border-amber-500/30 space-y-2">
                    {(() => {
                      const prize = drumRaffle.prizes.find((p) => p.id === selectedPrizeId);
                      if (!prize) return null;
                      return (
                        <div className="flex items-center justify-center space-x-3 pb-2 border-b border-white/10">
                          {prize.imageUrl ? (
                            <img
                              src={prize.imageUrl}
                              alt={prize.name}
                              className="w-12 h-12 rounded-xl object-cover border-2 border-amber-400 shadow-md shrink-0 cursor-pointer"
                              referrerPolicy="no-referrer"
                              onClick={() =>
                                setPreviewZoomImage({
                                  url: prize.imageUrl!,
                                  title: `${prize.place}º Lugar: ${prize.name}`,
                          ...
                          <div className="text-left">
                            <span className="text-[10px] uppercase font-black text-amber-300">
                              Premio a Entregar ({prize.place}º Lugar):
                            </span>
                            <h4 className="text-sm sm:text-base font-black text-white">{prize.name}</h4>
                          </div>
                        </div>
                      );
                    })()}
                    ...
                    {drumCandidateWinner.buyerPhone && (
                      <div className="flex items-center justify-center space-x-3 text-xs sm:text-sm mt-1.5">
                  ...
                  {/* DECISION AREA: PRESENT vs ABSENT / NO SHOW */}
                  <div className="pt-1 text-left">
                    <p className="text-xs font-black uppercase tracking-wider text-amber-200 text-center mb-2">
                      ¿La persona está presente para reclamar el premio?
                    </p>

                    {/* When showDisqualifyPrompt is false: Show Primary Action Choices */}
                    {!showDisqualifyPrompt ? (
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        {/* OPTION 1: CONFIRM WINNER */}
                        {isAdminOrLeader && (
                          <button
                            type="button"
                            onClick={handleConfirmWinner}
                            className="px-3 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-black text-xs shadow-lg shadow-emerald-600/30 flex flex-col items-center justify-center space-y-1 cursor-pointer active:scale-95 transition-all"
                          >
                            <Trophy className="w-5 h-5 text-amber-300" />
                            <span className="text-center font-bold">¡Sí está presente!</span>
                            <span className="text-[10px] text-emerald-200 font-normal">Proclamar Ganador</span>
                          </button>
                        )}

                        {/* OPTION 2: NOT PRESENT - EXCLUDE & RE-SPIN */}
                        <button
                          type="button"
                          onClick={() => setShowDisqualifyPrompt(true)}
                          className="px-3 py-3 rounded-xl bg-gradient-to-r from-rose-600 to-red-700 hover:from-rose-700 hover:to-red-800 text-white font-black text-xs shadow-lg shadow-rose-600/30 flex flex-col items-center justify-center space-y-1 cursor-pointer active:scale-95 transition-all"
                        >
                          <UserX className="w-5 h-5 text-rose-200" />
                          <span className="text-center font-bold">No se presentó</span>
                          <span className="text-[10px] text-rose-200 font-normal">Excluir y Re-sortear</span>
                        </button>

                        {/* OPTION 3: RE-SPIN WITHOUT EXCLUDING (REPEAT ALLOWED) */}
                        <button
                          type="button"
                          onClick={() => handleReSpinWithoutExcluding(true)}
                          className="px-3 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-200 font-black text-xs flex flex-col items-center justify-center space-y-1 cursor-pointer active:scale-95 transition-all"
                          title="Girar de nuevo manteniendo el boleto en la tómbola para que pueda volver a salir"
                        >
                          <RefreshCw className="w-5 h-5 text-blue-400" />
                          <span className="text-center font-bold">Girar sin excluir</span>
                          <span className="text-[10px] text-slate-400 font-normal">Permitir que se repita</span>
                        </button>
                      </div>
                    ) : (
                      /* DISQUALIFY / EXCLUSION CONFIGURATION CARD */
                      <div className="p-3.5 rounded-2xl bg-rose-950/70 border-2 border-rose-600 space-y-3 animate-fadeIn">
                        <div className="flex items-center justify-between">
                        ...
                        <div>
                          <label className="block text-[11px] font-bold text-rose-200 mb-1">
                            Motivo de descarte / exclusión del boleto:
                          </label>
                          <select
                        ...
                        <p className="text-[11px] text-rose-200">
                          ⚠️ Este número quedará <strong>excluido de este sorteo</strong> y <strong>no volverá a salir</strong>.
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 pt-1">
                          ...
                          >
                            <span>Solo Excluir (Sortear después)</span>
                          </button>
                        </div>
                      </div>
                    )}
            ...
            {/* ACTION BUTTONS (When no candidate winner is waiting for decision) */}
            {!drumCandidateWinner && (
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                <button
                  ...
                  disabled={isDrumSpinning}
                  className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white font-black text-base shadow-xl shadow-amber-500/30 transition-all flex items-center justify-center space-x-2 cursor-pointer active:scale-95 disabled:opacity-50"
                >
            ...
            {/* PRIZE PODIUM STATUS STRIP (GREAT FOR AUDIENCE PRESENTATION) */}
            <div className="pt-3 border-t border-slate-800 text-left">
              <p className="text-[11px] font-black uppercase tracking-wider text-slate-400 mb-2 flex items-center space-x-1.5">
                <Trophy className="w-3.5 h-3.5 text-amber-500" />
                <span>Estado de los Premios en este Sorteo:</span>
              </p>
                ...
                {drumRaffle.prizes.map((prz) => (
                  ...
                  >
                    {prz.imageUrl ? (
                      ...
                      <div className="w-8 h-8 rounded-lg bg-amber-500/20 text-amber-300 font-black text-xs flex items-center justify-center shrink-0">
                        {prz.place}º
                      </div>
                    )}
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[11px]">{prz.place}º Lugar</span>
                        {prz.winnerTicketNumber ? (
                          <span className="text-emerald-400 text-[10px] font-black">🏆 Ganado</span>
                        ) : (
                          <span className="text-[10px]">⏳ Por Sortear</span>
                        )}
                      </div>
                      <p className="font-black truncate text-white text-xs">{prz.name}</p>
                      {prz.winnerTicketNumber && (
                        <p className="text-[10px] text-emerald-400 truncate">
                          Boleto #{prz.winnerTicketNumber} {prz.winnerBuyerName ? `(${prz.winnerBuyerName})` : ''}
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <form onSubmit={handleSaveRaffleForm} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="Pro-Templo">Pro-Construcción & Templo</option>
                    <option value="Alabanza">Ministerio de Alabanza</option>
                    <option value="Jóvenes">Ministerio Juvenil</option>
                    <option value="Damas & Familia">Damas & Familias</option>
                    <option value="Misiones">Misiones & Evangelismo</option>
                    <option value="Navidad & Fiestas">Navidad & Fiestas</option>
                    <option value="Benéfico">Acción Social & Caridad</option>
                    <option value="General">General Eclesial</option>
                  </select>
                </div>
